...
class Apriori{
    ...
    public function setUseTextOrArray($char){
      $this->type=$char;
    }    

    public function getUseTextOrArray(){
      return $this->type; 
    }
    ...
    **/
    ...
    { 
       ...
       for($i=0; $i<$num; $i++) 
          {
             if($this->getUseTextOrArray() == 'text'){
                $tmp  = explode($this->delimiter,$db[$i]);
             }else if($this->getUseTextOrArray() == 'array'){
                 $tmp  = $db[$i];
             }
             $num1 = count($tmp);
             $x    = array();
       ...
       foreach($this->freqItmsts as $k => $v)
          {
             ...
             for($i=0; $i<$num; $i++)
                {  
                   $tmp[] = $this->realName($k[$i]);//line no. 227 
